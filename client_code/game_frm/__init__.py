...
class game_frm(game_frmTemplate):
    def __init__(self, parent_form, **properties):
        ...
        self.item["interval_tmr"]  = 0  # stop timer first
    ...
    def start_btn_click(self, **event_args):
        ...
        if self.start_btn.text in "Play again":
            ...
            self.item["interval_tmr"] = 1  # start at 1 second per tick
            # set start-game parameters
            self.item["time"]         = 25
            self.item["previous"]     = "X"
            self.item["score"]        = 0
            self.item["target_count"] = 0
            self.item["instruction"]  = (
                f"Click on the letter '{self.item['target_letter']}' as many times as you can in 25 seconds."
            )
            self.refresh_data_bindings()
    ...
    def game_tmr_tick(self, **event_args):
        ...
        if self.item["start_timer"]:  # game started
            ...
            if self.item["time"] == 0:
                ...
                # show player card from parent form: <My_Clicky_Game_2.player_frm.player_frm object>
                ...
                # refresh history list in parent form
                self.parent_form.refresh_bindings()

                # go to the next level: adjust some game parameters
                self.item["level"]        += 1
                self.item["interval_tmr"] -= self.item["speed"]
                # self.game_tmr.interval        -= self.item['speed']
                self.item["target_letter"] = self.get_random_letter(
                    target=True
                )  # randomly change target letter for subsequent level
                ...
                if self.game_tmr.interval == 0:
                    if alert(
                        "Congratulations! You've completed all game levels.\nDo you want to start over",
                        buttons=[("Yes", True), ("No", False)],
                    ):
                        # self.game_tmr.interval = 1
                        self.item["interval_tmr"] = 1
                # refresh data bindings for timer interval
                self.refresh_data_bindings()
